...
/*Récupère une liste a apartie de api  et remplit un tableau avec leurs détails*/
const quranPage = () => {
    //Quran.html /**consommation api  */
    $.get('https://api.alquran.cloud/v1/surah', (data) => {
        isiDaftar(data.data);
        $('.loading').css('display', 'none');
    });

/*bech ne5th nom et nbr aya bengligh*/
    const isiDaftar = (data) => {
        let i = 1;
        data.forEach((d) => {
            const elemenList = `<tr  class="">
                            <th class="td1" width="30"><div class="no_s">${i}</div> </th>
                            <td class="td2 tebal nama_s" no-surah="${d.number}">${d.name}<br>${d.englishName}</td>
...
/*Récupère et affiche des informations sur une sourate spécifique,arab text  lecture audio */
const surahPage = () => {
    //Surah.html
    const url = window.location.href;
    let no_s = '';
...
/*get  surah api arab*/
    const url_api = `https://api.alquran.cloud/v1/surah/${no_s}/editions/ar.alafasy,en.transliteration,id.arabe`;
    let surah = [];
    let jmlAyah;
    ...
    $.get(url_api, (data) => {
        ...
        $('.loading').css('display', 'none');
        $('.judul').text(`Surah ${data.data[0].name}`);
